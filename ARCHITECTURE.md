...
| 视觉 | SVG（SVG Path + ClipPath + Pointer Events） | 36 块拼图由固定模板 + ClipPath 显示原图，锁定占位、解锁 reveal、自由拖拽/吸附/连块 |
...
- 植物拼图视觉：一张完整植物插画（PNG，1:1）配合固定 6×6 共 36 块的 SVG Path 模板，通过 SVG ClipPath 让每块显示原图对应区域（不提前切 36 张图）。锁定块显示米白半透明占位（不透露原图），解锁块显示原图区域并播放柔和 reveal；完成 36/36 后可进入自由拼图（Pointer Events 拖动 + 邻块 Snap + Group 连块）。参考纹理见 `references/`。

> **植物素材**：首批 3 株植物均为 PNG（`public/plants/*.png`），依次为 仙人掌-边界（cactus_boundary）、蘑菇-平静（mushroom_calm）、蒲公英-放下（dandelion_release），其中「仙人掌」使用用户上传的插画（`references/cactus-plant.png`）；「拼图纹理」参考灰度纹理图（`references/puzzle-texture-reference.png`），锁定态由 Shader 灰度+压暗实现。
