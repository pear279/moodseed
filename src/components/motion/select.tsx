...
export function SelectContent({ className, children }: SelectContentProps) {
  ...
  return (
    <motion.div
      ...
      transition={
        ctx.reduce
          ...
          : {
              ...
              height: open ? { type: 'spring', duration: 0.42, bounce: 0.14 } : { duration: 0.26, ease: EASE_OUT, delay: 0.14 },
      ...
      style={{ transformOrigin: isTop ? 'bottom' : 'top', overflow: 'hidden', pointerEvents: open ? 'auto' : 'none' }}
      className={cn(
    ...
    <motion.li variants={ctx.reduce ? undefined : ITEM_VARIANTS}>
      <button
        ...
        className={cn(
          ...
          selected ? 'bg-muted text-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground focus-visible:bg-muted',
          'disabled:pointer-events-none disabled:opacity-50',
          className,
        )}
      >
