...
- [~] 3.6 规则单测/脚本校验（以 curl 手测代替自动化单测）

## Phase 4 — 植物拼图核心视觉 ✅（CSS 3D 实现）
- [x] 4.1 6×8 切片：`background-position` 切片，未解锁灰暗+蜘蛛网，解锁彩色
- [x] 4.2 三态渲染：完全未解锁 / 部分恢复 / 完整复苏
- [x] 4.3 卡片倾斜视差：`TiltCard`（CSS 3D，替代 Three.js，轻量）
- [x] 4.4 碎片恢复动画：灰→彩逐块点亮 + 「🌱 获得 N 块」反馈
- [~] 4.5 完整解锁：翻牌 + 光晕 + 植物卡片 + 「收下」（基础浮起动画，翻牌为简化版）
